...
from hiad.detectors import HRINPFormer
...
If you use DINOv3, make sure to correctly set the path to the DINOv3 weights in the `configs/inpformer.yaml` file.

You can apply for the weights at the following URL:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--thumbnail_config", default = 'configs/inpformer.yaml', type=str, help='Low-res(thumbnail) detection model configuration path')
    parser.add_argument("--detector_config_path", default = 'configs/inpformer.yaml', type = str, help='High-res detection model configuration path')

    parser.add_argument("--detector_number", default = 1, type=int, help='number of detector')
    parser.add_argument("--evaluation_only", default = False, type=bool, help='Run evaluation only')
    parser.add_argument("--early_stop_epochs", default = 4, type = int,
    ...
    parser.add_argument("--checkpoint_root", default ='results/inpformer_checkpoints', type = str, help='Directory for saving model checkpoints')
    parser.add_argument("--log_root", default = 'results/inpformer_logs', type = str, help='Directory for saving logs')
    parser.add_argument("--vis_root", default = 'results/inpformer_vis', type = str, help='Directory for saving visualization results')
    parser.add_argument("--gpus", default="0,1", type=str, help='gpu ids')
    ...
    if not args.evaluation_only:
        ...
        tasks = A2OTaskGenerator(device=args.gpus).create_tasks(train_samples, image_indexes)
